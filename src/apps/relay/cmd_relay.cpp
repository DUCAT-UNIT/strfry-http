...
void RelayServer::run()
{
    ...
    // Initialize all thread pools FIRST
    ...
    // NOW start HTTP thread AFTER all thread pools are initialized
    // This ensures tpIngester is ready before HTTP events can be dispatched
    if (cfg().relay__http__enabled)
    {
        ...
        httpThread = std::thread([this]()
                                 {
            try {
                // Small delay to ensure thread pools are fully ready
...
void RelayServer::runHttpServer()
{
    ...
    httpServer = std::make_unique<HttpServer>(
        ...
        env.lmdb_env,      // Use the lmdb_env from the environment
        env.dbi_Event__id, // Use the id index for looking up events
    ...
    // Start the HTTP server (this blocks, so it runs in its own thread)
